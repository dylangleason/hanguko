...
- Build queries in the context's `Queries` module (`Hanguko.Content.Queries`, `Hanguko.SRS.Queries`, `Hanguko.Progress.Queries`, `Hanguko.Audio.Queries`), never in the context itself. Query modules return `Ecto.Query` structs and never call `Repo`; contexts run them and own every transaction and lock. Name bindings with `as:` and have narrowing functions (`query -> query`) match on the binding name, not position. Contexts don't `import Ecto.Query`. `Hanguko.Accounts` is generated code and is exempt. See "Queries" in `guides/architecture.md`
- Migrations use `:text` for string columns, never `:string`. Ecto's `:string` creates `varchar(255)`, an arbitrary limit that fails as a database error rather than a changeset error. In Postgres, `text` performs the same. Put a real length rule in the changeset with `validate_length`, plus a named `CHECK` constraint if the database must enforce it too. Existing `varchar(255)` columns are left as they are. Schema fields stay `:string` either way
...
- Tailwindcss v4 **no longer needs a tailwind.config.js** and uses a new import syntax — see `assets/css/app.css` for the current setup. **Always** maintain that import syntax rather than reverting to a `tailwind.config.js`
