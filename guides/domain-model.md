...
| `reps`, `lapses` | Total reviews, and times a review card was rated Again. Eight lapses make a card a leech. |
| `suspended` | Out of the queue without losing history. Set by hand in the card browser, or automatically once a card becomes a leech. |
...
* **A leech is set aside, never deleted.** Eight lapses suspend a card, and
  it stays flagged as a leech afterwards because that is read from `lapses`
  rather than stored; resetting it in the card browser is what clears the
  count.
* **Resetting a card keeps its row**, so its reviews stay valid. It is the
  one way a card can have reviews behind it and no `last_review_at`.
* **A phrase deck is a situation.** Every phrase has a `politeness` of
  ...
  in the same deck, by its full `source_key`, from one side of the pair only. The importer rejects either
  rule being broken.
